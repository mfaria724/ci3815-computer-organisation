...
bloqueado: .asciiz "Computador está bloqueado\n"
spacebar: .word 0
...
men_bloqueo:lw $t4, spacebar
	bnez $t4, desbloqueo
	lw $t0, 8($s0)
	andi $t0, $t0, 0x1
	beq $t0, $0, men_bloqueo
	lb $t3, bloqueado($t2)
	addi $t2, $t2, 1
	sw $t3, 12($s0)

	
	ble $t2, $t1, men_bloqueo
	li $t2, 0
	b men_bloqueo
...
men_desbloqueo:
	...
	lw $t0, 8($s0)
	andi $t0, $t0, 0x1
	beq $t0, $0, men_desbloqueo
	lb $t3, desbloqueado($t2)
	addi $t2, $t2, 1
	sw $t3, 12($s0)
	
	ble $t2, $t1, men_desbloqueo
